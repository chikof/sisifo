...
<div class="node-stats">
  <div class="header">
    ...
    <button class="refresh-btn" on:click={loadAll} disabled={loading} title="Refresh">
  ...
  <div class="section">
    ...
    <div class="identity-card">
      ...
      <div class="id-row">
        ...
        {#if nodeIdentity}
          <button class="copy-btn" on:click={copyId} title="Copy node ID">
            {#if copiedId}
      ...
      <p class="id-hint">This is your persistent public identity on the network. Derived from your ed25519 keypair.</p>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="daemon-card" class:online={daemonRunning}>
      <div class="daemon-status">
        ...
        <span class="status-label">{daemonRunning ? 'sisid is running' : 'sisid is offline'}</span>
      </div>
      <p class="daemon-desc">
        {#if daemonRunning}
          Sites are being seeded persistently in the background, even when this app is closed.
        {:else}
          The daemon is not running. Sites will only be seeded while this app is open. Enable sisid via systemd to seed persistently.
        {/if}
      </p>
      {#if !daemonRunning}
...
<style>
  ...
  .header p { font-size: 13.5px; color: var(--text-2); }
  ...
  .id-label { font-size: 12px; font-weight: 500; color: var(--text-2); }
  ...
  .id-hint { font-size: 12px; color: var(--text-3); line-height: 1.5; }
  ...
  .daemon-card.online { border-color: color-mix(in srgb, var(--accent) 30%, var(--border)); }

  .daemon-status { display: flex; align-items: center; gap: 8px; }
  .status-dot {
    width: 8px;
    height: 8px;
    border-radius: 50%;
  ...
  .status-label { font-size: 13.5px; font-weight: 500; color: var(--text); }
  .daemon-desc { font-size: 13px; color: var(--text-2); line-height: 1.55; }
  .daemon-cmd {
    display: block;
    font-family: var(--mono);
    font-size: 12px;
